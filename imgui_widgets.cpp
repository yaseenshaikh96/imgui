...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    const ImVec2 label_pos(pos.x + padding.x + ImMax(0.0f, (label_avail_w - label_size.x) * style.SeparatorTextAlign.x), pos.y + text_baseline_y); // FIXME-ALIGN
...
void ImGui::SeparatorText(const char* label)
{
    ...
    SeparatorTextEx(0, label, FindRenderedTextEnd(label));
}
